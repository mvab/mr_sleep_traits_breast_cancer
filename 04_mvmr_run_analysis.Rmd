...
mvmr_input <- make_mvmr_input(exposure_dat, exposure_list, outcome.id.mrbase=breast_cancer_id)
...
for (i in 1:length(mediators)){
  ...
  print("=========== Running analysis 2 =============")
    ...
    mv_res<- res$result %>%
                  ...
                  generate_odds_ratios() %>% 
                  select(-id.exposure, -id.outcome)
    write_tsv(mv_res, paste0(mvmr_dir, "mvmr_earlyBMI-", mediator_file_name ,"-to-BCAC_", bc_data,"_using_2SMR.tsv"))
  ...
  #  estimate causal effects using method in MVMR package
    ...
    write_tsv(mvmr_res, paste0(mvmr_dir, "mvmr_earlyBMI-", mediator_file_name ,"-to-BCAC_", bc_data,"_using_MVMR.tsv"))
    ...
    if (mediator_name %in% phenocor_values$mediator){
      ...
      #estimate gencov
      gen_cov <- phenocov_mvmr(Pcov = pheno_mat, seBXGs = se_matrix)
    } else{
      gen_cov <- 0
    }
    ...
    write_tsv(mvmr_sens_df, paste0(mvmr_dir, "mvmr_sens_earlyBMI-", mediator_file_name ,"-to-BCAC_", bc_data,".tsv"))
  ...
  if (run_analysis_3) {
    ...
    mv_res<- res$result %>%
                  ...
                  generate_odds_ratios() %>% 
                  select(-id.exposure, -id.outcome)
    write_tsv(mv_res, paste0(mvmr_dir, "mvmr_adultBMI-", mediator_file_name ,"-to-BCAC_", bc_data,"_using_2SMR.tsv"))
  ...
  #  estimate causal effects using method in MVMR package
    ...
    write_tsv(mvmr_res, paste0(mvmr_dir, "mvmr_adultBMI-", mediator_file_name ,"-to-BCAC_", bc_data,"_using_MVMR.tsv"))
    ...
    write_tsv(mvmr_sens_df, paste0(mvmr_dir, "mvmr_sens_adultBMI-", mediator_file_name ,"-to-BCAC_", bc_data,".tsv"))
 ...
 # Analysis 4. Multivariable MR: Childhood BMI, Adult BMI, and Mediator as exposures, Breast cancer as outcome
  ...
  print("=========== Running analysis 4 =============")
    ...
    write_tsv(mv_3exp, paste0(mvmr_dir, "mvmr_adultBMIearlyBMI-", mediator_file_name,"-to-BCAC_", bc_data,"_using_2SMR.tsv"))
  ...
  #  estimate causal effects using method in MVMR package
    ...
    write_tsv(mvmr_res, paste0(mvmr_dir, "mvmr_adultBMIearlyBMI-", mediator_file_name ,"-to-BCAC_", bc_data,"_using_MVMR.tsv"))
    ...
    if (mediator_name %in% phenocor_values$mediator){
      print(paste0("Using phenocor for calculating Fst"))
      phenocor_values <- filter(phenocor_matrix, mediator %in% c(mediator_name, "Childhood body size","Adult body size"))
      phenocor_values$mediator_col <- c(as.numeric(phenocor_values[3,2]), as.numeric(phenocor_values[3,3]), 1)
      pheno_mat <- phenocor_values %>% select(-mediator) %>% as.matrix()
      colnames(pheno_mat) = rownames(pheno_mat) = c("Childhood body size","Adult body size", mediator_name)
      print(pheno_mat)
      #extract SE matrix
      se_matrix <- mvmr_input$XGs %>% select(contains("se.")) %>% as.data.frame()
    ...
    #Test for weak instruments
    sres <- strength_mvmr(r_input=mvmr_out, gencov=gen_cov)
    colnames(sres) = paste(c("Childhood body size","Adult body size", mediator_name), "(Fst)")
    print(sres)
    ...
    write_tsv(mvmr_sens_df, paste0(mvmr_dir, "mvmr_sens_adultBMIearlyBMI-", mediator_file_name ,"-to-BCAC_", bc_data,".tsv"))
...
# this is a liust of MVMR types to cycle through
...
for (mvmr_type in mvmr_types) {
  ...
  # exclude anay files with prefic 'merged'
  mvmr_files <- mvmr_files[!grepl('merged', mvmr_files )]
  # select type
  mvmr_files_type <- mvmr_files[grepl(mvmr_type, mvmr_files )]
  # select 2SMR results
  mvmr_files_type <- mvmr_files_type[grepl(mvmr_method, mvmr_files_type )]
  # for mvmr with breast cancer, subset to the correct type
  if (mvmr_type != "mvmr_BMIs-"){
      mvmr_files_type <- mvmr_files_type[grepl(bc_data, mvmr_files_type)] 
  }
  ...
  write_tsv(df, paste0(results_path, current_trait_category, "/mvmr/merged_", file_prefix, "_", mvmr_method, ".tsv"))
